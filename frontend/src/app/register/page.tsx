...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Güçlü Kurumsal Şifre Doğrulaması
    if (password.length < 8) {
      setError('Şifreniz en az 8 karakter uzunluğunda olmalıdır.');
      return;
    }
    if (!/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password) || !/[^a-zA-Z0-9]/.test(password)) {
      setError('Şifreniz en az bir büyük harf, bir küçük harf, bir rakam ve bir özel karakter (*, !, @, # vb.) içermelidir.');
      return;
    }
    ...
    try {
      ...
      if (response.data.success && response.data.accessToken && response.data.refreshToken && response.data.user) {
        ...
        setError(response.data.message || 'Kayıt yapılamadı.');
      }
    } catch (err: any) {
      if (err.response?.status === 429) {
        setError('Çok fazla işlem denemesi yapıldı. Güvenliğiniz için lütfen 1 dakika bekleyip tekrar deneyin.');
      } else {
  ...
  const isWarningError = error?.includes('karakter') || error?.includes('Şifreniz');
  const isRateLimitError = error?.includes('bekleyip') || error?.includes('denemesi');
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between items-center relative overflow-hidden bg-slate-950 text-slate-100 selection:bg-indigo-500 selection:text-white">
      ...
      {/* Üst / Orta: Merkezlenmiş Kayıt Kartı */}
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md px-4 sm:px-6 my-auto relative z-10 space-y-6 pt-10 pb-6">
        ...
        <div className="bg-slate-900/85 backdrop-blur-2xl border border-slate-800/80 rounded-3xl p-7 sm:p-8 shadow-2xl shadow-black/80 w-full transition-all">
          <div className="mb-6">
            <h2 className="text-xl font-bold text-white tracking-tight">Yeni Hesap Oluştur</h2>
            <p className="text-slate-400 text-xs mt-1">Sisteme hemen kaydolun ve finansal özgürlüğünüzü yönetin</p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  placeholder="Min 8 karakter (A-z, 0-9, sembol)"
